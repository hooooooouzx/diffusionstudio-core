...
import { FPS_DEFAULT, framesToMillis, Timestamp } from '../models';
...
export class Composition extends EventEmitterMixin<CompositionEvents, typeof Serializer>(Serializer) {
	...
	private async ticker() {
		const interval = 1000 / FPS_DEFAULT; // target frame duration in ms
		...
		while (this.frame <= this.duration.frames && this.playing) {
			...
			// compute how many frames should have passed
			let framesToAdvance = Math.floor(elapsed / interval);
			if (framesToAdvance > 0) {
				for (let i = 0; i < framesToAdvance; i++) {
					this.computeFrame();
				}
				lastTime += framesToAdvance * interval;
			}
		}
